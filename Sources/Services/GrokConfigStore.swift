...
/// Manages Grok Build skill enable/disable state via `~/.grok/config.toml`.
///
/// Grok uses the same `[[skills.config]]` TOML table format as Codex for
/// per-skill overrides (path + enabled = false). This store provides the
/// same API surface as CodexConfigStore so SkillManager can treat it uniformly.
struct GrokConfigStore: Sendable {
...
// MARK: - Internal types (shared parsing structures with CodexConfigStore)
